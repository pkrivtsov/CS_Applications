...
def checking_data(message):
    ...
    if action not in dict_of_commands:
        return {'response': 404, 'time': time.time(), 'error': 'Неизвестная команда'}
...
def authenticate(*kwargs):  # пароль не запрашивается
    ...
    return {'response': 200, 'time': time.time(), 'alert': f'Пользователь {user_name} аут-н'}
...
def msg(**kwargs):
    ...
    if from_user not in authorized_users:
        return {'response': 401, 'time': time.time(), 'error': f'Пользователь {from_user} не авторизован'}

    if to_user[0] == '#':
        chat = to_user[1:]
        if chat not in chat_rooms:
            return {'response': 404, 'time': time.time(), 'error': f'Чат {chat} не найден'}
        return {
            'response': 200,
            'time': time.time(),
    ...
    if to_user not in authorized_users:
        return {'response': 404, 'time': time.time(), 'alert': f'Пользователь {to_user} не авторизован'}
...
def quit_s(**kwargs):
    ...
    if user_name in authorized_users:
        ...
        return {'response': 200, 'time': time.time(), 'alert': 'Пользователь {user_name} успешно вышел'}
    return {'response': 404, 'time': time.time(), 'error': 'Пользователь {user_name} не найден'}
...
def create(**kwargs):
    ...
    return {'response': 200, 'time': time.time(), 'alert': f'Чат {chat_name}'}
